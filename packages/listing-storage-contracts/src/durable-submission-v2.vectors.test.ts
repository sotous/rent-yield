...
describe("DurableSubmissionV2 canonical vectors", () => {
  ...
  it("keeps replay scoped by source and provides valid conflict inputs", () => {
    ...
    for (const vector of Object.values(vectors))
      expect(durableSubmissionV2Schema.safeParse(vector).success).toBe(true);
    expect(vectors.exact_replay.submission_id).toBe(
      vectors.same_submission_id_other_source.submission_id,
    );
    expect(vectors.exact_replay.source_key).not.toBe(
      vectors.same_submission_id_other_source.source_key,
    );
    expect(vectors.exact_replay.capture.capture_event_id).not.toBe(
      vectors.capture_event_conflict.capture.capture_event_id,
    );
    expect(vectors.exact_replay.interpretation).not.toEqual(
      vectors.interpretation_conflict.interpretation,
    );
  });
  ...
  it("parses immutable accepted receipts, ordered progress, and sanitized errors", () => {
    expect(
      acceptedReceiptV2Schema.safeParse(acceptedReceiptV2Vector).success,
    ).toBe(true);
  ...
  it("makes accepted-submission hash inclusions and exclusions explicit", () => {
    ...
    expect(acceptedSubmissionDigestV2(vectors.excluded_submission_id)).toBe(
      baseline,
    );
    expect(acceptedSubmissionDigestV2(vectors.excluded_submitted_at)).toBe(
      baseline,
    );
    for (const vector of [
      vectors.included_capture,
      vectors.included_typed_outcome,
      vectors.included_provenance,
      vectors.included_interpretation,
    ])
